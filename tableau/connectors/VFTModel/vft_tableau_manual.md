...
> **Nota Fase 3:** `b_puntos` fue migrado a `geo_layers.py` — ahora devuelve una FeatureCollection estándar vía `/geolayers/betweenness?layer=b_puntos`. `t_escalar` sigue en `main.py` y no es FeatureCollection (estructura propia documentada en §6.6).
...
Fuente: `GET /api/v1/network/geolayers/betweenness?layer=b_puntos&limit=2000`

Devuelve una **FeatureCollection estándar** (igual que el resto de GeoLayers). El WDC usa `rowsFromFeatures` y extrae lat/lon de `geometry.coordinates`.
...
```json
{
  ...
  "features": [
    {
      "type": "Feature",
      "geometry": { "type": "Point", "coordinates": [-99.187, 19.401] },
      "properties": {
        "id": "...", "nombre": "Tacubaya", "sistema": "METRO",
        "alcaldia_municipio": "Miguel Hidalgo", "es_cetram": true,
        "betweenness_centrality": 0.2179
      }
    }
  ]
}
...
| Columna WDC | Tipo | Campo en `properties` | Descripción |
|-------------|------|-----------------------|-------------|
| `node_id` | string | `id` | ID interno del nodo en el grafo |
| `nombre` | string | `nombre` | Nombre de la estación |
| `sistema` | string | `sistema` | METRO, MB, RTP… |
| `alcaldia_municipio` | string | `alcaldia_municipio` | Alcaldía o municipio |
| `es_cetram` | bool | `es_cetram` | Si la estación es CETRAM |
| `betweenness_centrality` | float | `betweenness_centrality` | B(v) normalizado [0–1] |
| `lat` | float | extraído de `geometry.coordinates[1]` | Latitud WGS84 |
| `lon` | float | extraído de `geometry.coordinates[0]` | Longitud WGS84 |

> **Primera ejecución:** 30–90 s (algoritmo de Brandes sobre ~10,561 nodos). Con caché: <100 ms.
